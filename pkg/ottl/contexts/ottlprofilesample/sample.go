...
var (
	...
	_ internal.SampleContext               = TransformContext{}
)
...
func NewTransformContext(sample pprofile.Sample, profile pprofile.Profile, instrumentationScope pcommon.InstrumentationScope, resource pcommon.Resource, scopeProfiles pprofile.ScopeProfiles, resourceProfiles pprofile.ResourceProfiles, options ...TransformContextOption) TransformContext {
	tc := TransformContext{
		sample:               sample,
		profile:              profile,
...
func (tCtx TransformContext) GetSample() pprofile.Sample {
	return tCtx.sample
}
...
func (pep *pathExpressionParser) parsePath(path ottl.Path[TransformContext]) (ottl.GetSetter[TransformContext], error) {
	...
	if path.Context() == "" {
		switch path.Name() {
		case internal.ResourceContextName, internal.InstrumentationScopeContextName, internal.ProfileContextName:
	...
	default:
		return internal.SamplePathGetSetter[TransformContext](path)
	}
}
...
func (pep *pathExpressionParser) parseHigherContextPath(context string, path ottl.Path[TransformContext]) (ottl.GetSetter[TransformContext], error) {
	...
	case internal.ProfileContextName:
		return internal.ProfilePathGetSetter(path)
	default:
		var fullPath string
		if path != nil {
			fullPath = path.String()
		}
		return nil, internal.FormatDefaultErrorMessage(context, fullPath, contextNameDescription, internal.SampleRef)
	}
}
